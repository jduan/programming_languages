...
in
	if real_equal(a,c) andalso real_equal(b,d)
	then (print "eval_prog with shift line worked\n")
	else (print "eval_prog with shift line is not working properly!!!\n")
end;
...
in
	if real_equal(a,b)
	then (print "eval_prog with shift VerticalLine worked\n")
	else (print "eval_prog with shift VerticalLine is not working properly!!!\n")
end;

let
	val LineSegment(a, b, c, d) = (eval_prog (preprocess_prog (Shift(3.0, 4.0, LineSegment(1.0, 2.0, 3.0, 4.0))), []))
	val LineSegment(e, f, g, h) = LineSegment(4.0, 6.0, 6.0, 8.0)
in
	if real_equal(a,e) andalso real_equal(b,f) andalso real_equal(c,g) andalso
	real_equal(d,h)
	then (print "eval_prog with shift LineSegment worked\n")
	else (print "eval_prog with shift LineSegment is not working properly!!!\n")
end;
